...
namespace aeon.Integration1C.Server
{
  public class ModuleAsyncHandlers
  {
    ...
    public virtual void SendContractualDocsIn1CAsync(aeon.Integration1C.Server.AsyncHandlerInvokeArgs.SendContractualDocsIn1CAsyncInvokeArgs args)
    {
      ...
      foreach (var document in contractualDocs)
      {
        ...
        try
        {
          ...
          Logger.Error(json);
          Functions.Module.SendMessageFromRabbitMQ(hostName, virtualHost, userName, password, json, exchange, routingKey);
    ...
    public virtual void GetContractualDocIn1CAsync(aeon.Integration1C.Server.AsyncHandlerInvokeArgs.GetContractualDocIn1CAsyncInvokeArgs args)
    {
      var setting = Functions.SettingsIntegration.GetSettingsIntegration();
      var queueName = setting.QueueGetForContracts;
      var verificationQueue = setting.QueueSendForVerification;
      ...
      foreach (var message in messages)
      {
        ...
        var result = Functions.Module.DeserializeMessageFromContractualDoc(message);
        ...
        try
        {
          ...
          else if (setting.ContractResponsible != null && setting.ContractResponsible.Department != null)
          {
            contractualDoc.ResponsibleEmployee = setting.ContractResponsible;
            contractualDoc.Department = setting.ContractResponsible.Department;
          }
          ...
          var messageResult = Functions.Module.SerializeMessageFromResult(contractualDoc.Id, contractualDoc.Guid1C, result.correlation_id, verificationQueue);
          Functions.Module.SendMessageFromRabbitMQ(hostName, virtualHost, userName, password, messageResult, exchange, routingKeyVerification);
        }
        catch (Exception ex)
        {
          Logger.Error(ex.Message, ex);
          var messageError = Functions.Module.SerializeMessageFromError(ex.Message, result.correlation_id, errorQueue);
          Functions.Module.SendMessageFromRabbitMQ(hostName, virtualHost, userName, password, messageError, exchange, routingKeyError);
        }
      }
    }
    ...
    public virtual void GetCompanyIn1CAsync(aeon.Integration1C.Server.AsyncHandlerInvokeArgs.GetCompanyIn1CAsyncInvokeArgs args)
    {
      var setting = Functions.SettingsIntegration.GetSettingsIntegration();
      ...
      foreach (var message in messages)
      {
        ...
        var result = Functions.Module.DeserializeMessageFromCompany(message);
        var resultBody = result.body;
        ...
        /* var oldCompany = (resultBody.Type == aeon.Integration1C.Resources.NaturalPersonCounterpartyKind
                          || resultBody.Type == aeon.Integration1C.Resources.SelfEmployedCounterpartyKind)
          ? (!string.IsNullOrEmpty(resultBody.TIN)
             ? companies.Where(c => c.TIN == resultBody.TIN).FirstOrDefault()
             : companies.Where(c => c.Name == resultBody.Name).FirstOrDefault())
          : companies.Where(c => c.Guid1C == resultBody.Guid ||
                            resultBody.Nonresident != null && ((resultBody.Nonresident == false && c.TIN == resultBody.TIN && c.TRRC == resultBody.TRRC) ||
                                                               (resultBody.Nonresident == true && c.RegNumber == resultBody.RegNumber))).FirstOrDefault();
         */
        ...
        try
        {
          ...
          company.Save();
          var messageResult = Functions.Module.SerializeMessageFromResult(company.Id, company.Guid1C, result.correlation_id, verificationQueue);
          Functions.Module.SendMessageFromRabbitMQ(hostName, virtualHost, userName, password, messageResult, exchange, routingKeyVerification);
        }
        catch (Exception ex)
        {
